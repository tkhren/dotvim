...
function! s:load_my_plugins(dein_repository, dein_base_dir) abort "{{{1
    ...
    if dein#tap('vim-substy') "{{{2
        nn <expr> [sub] substy#substitute_operator('\m')
        no <expr> [sub]s substy#substitute('\m', '', '')
        no <expr> [sub]/ substy#substitute('', @/, '')
        no <expr> [sub]? substy#substitute('', @/, @/)
        no <expr> [sub]g substy#global('', @/, '')
        no <expr> [sub]v substy#vglobal('', @/, '')
        no <expr> [sub]y substy#yank()
        for c in split('abcdefghijklmnopqrstuvwxyz0123456789*+.%/"', '\zs')
            let mapcmd = 'no <expr> [sub]"%s substy#substitute("\\m", @%s, "")'
